async function calculateRisk(user, guild) {
  ...
  if (ageDays < 1) {
    ...
    score += 70;
    factors.push('⚠️ Account less than 3 days old');
  } else if (ageDays < 7) {
    score += 50;
    factors.push('⚠️ Account less than 1 week old');
  } else if (ageDays < 30) {
    score += 25;
    factors.push('⚠️ Account less than 1 month old');
  } else if (ageDays < 90) {
    score += 10;
  ...
  try {
    ...
    if (joinAge < 1000 * 60 * 10) { // Less than 10 minutes
      ...
      score += 25;
      factors.push('⏱️ Joined server very recently (< 1 hour)');
    ...
    if (member.permissions.has('Administrator')) {
      ...
      score -= 15;
      positiveIndicators.push('🛡️ Has moderation permissions');
    }
